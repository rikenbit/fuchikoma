...
pairs(result.pca.MARS$rotation[, 1:10], col=label.MARS)
@
\end{center}

% PCAの線形性を解消すべく、近年では非線形な次元圧縮が開発されてきた
% 以下は、比較的scRNA-Seq分野で利用されている、t-SNEとdiffusionMapの結果であるが、非線形性により、細胞腫を分離できていることがわかる
...
\begin{center}
<<echo=TRUE, print=FALSE, include=TRUE, fig=TRUE, width=12, height=12>>=
pairs(result.destiny.MARS@eigenvectors[, 1:10], col=label.MARS)
@
\end{center}
