...
L_rand <- CatKernel(sample(1:10, replace=TRUE, 10), type="one_vs_rest")
res.rand <- HSIC(K_rand, L_rand, type="gamma")
...
L_cell <- CatKernel(label, type="simple")
res.cell <- HSIC(K_cell, L_cell, type="gamma")

# DEGs matrix → HSIC (Ultra High value) & Pvalue (Ultra Low value)
K_deg <- as.matrix(custom.DiffusionMap(t(testdata[1:30,]))$M)
L_deg <- CatKernel(label, type="simple")

res.deg  <- HSIC(K_deg, L_deg, type="gamma")
